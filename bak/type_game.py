...
engine.say("hello,baby,let's play games,please fill the word as the picture showed!")
engine.runAndWait()
#读入图片库中的所有图片
images=[]
...
def func():
    ...
    if n==Max_num-1:
        ...
        if e.get()!=filenames[n]:#利用Variable()获取entry内部text
            ...
            e.set("")
            picc=PhotoImage(file="./pic/"+images[n+1])
...
#显示图片
pic=PhotoImage(file="./pic/"+images[0])
imageLabel=Label(window,image=pic)
imageLabel.place(x=200,y=0)

win_pic=PhotoImage(file="./win/结衣.png")
